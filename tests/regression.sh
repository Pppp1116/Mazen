...
ORIG_NS="$(python - <<'PY2' "$AUTO_LIB_HASH/src/main.c"
import os, sys
...
SRC
python - <<'PY2' "$AUTO_LIB_HASH/src/main.c" "$ORIG_NS"
import os, sys
path = sys.argv[1]
ns = int(sys.argv[2])
...
python - <<'PY2' "$COMPDB/compile_commands.json"
import json, sys
p=sys.argv[1]
...
BASE_SEC="$(python - <<'PY2' "$SAMESEC/src/main.c"
import os, sys
...
SRC
python - <<'PY2' "$SAMESEC/src/main.c" "$BASE_SEC" 100
import os, sys
path = sys.argv[1]
sec = int(sys.argv[2])
nsec = int(sys.argv[3])
...
SRC
python - <<'PY2' "$SAMESEC/src/main.c" "$BASE_SEC" 200
import os, sys
path = sys.argv[1]
sec = int(sys.argv[2])
nsec = int(sys.argv[3])
...
python - <<'PY2' "$CDBX/compile_commands.json"
import json, sys
