...
static const char *AUTO_FFMPEG_LIBRARIES_HEADERS[] = {"libavcodec/avcodec.h"};
...
static const AutoLibRule AUTO_LIB_RULES[] = {
    ...
    {AUTO_FFMPEG_LIBRARIES_LIBS, MAZEN_ARRAY_LEN(AUTO_FFMPEG_LIBRARIES_LIBS), AUTO_FFMPEG_LIBRARIES_HEADERS, MAZEN_ARRAY_LEN(AUTO_FFMPEG_LIBRARIES_HEADERS), NULL, 0, NULL, 0},
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) {
        ...
        while (*cursor != '\0') {
            ...
            if (cursor[0] == '-' && cursor[1] == 'l') {
                ...
                *cursor = '\0';
                if (*name != '\0') {
                    string_list_push_unique(libs, name);
                }
                *cursor = saved;
            } else {
                while (*cursor != '\0' && !isspace((unsigned char) *cursor)) {
...
static bool package_matches_token(const char *package, const char *token) {
    ...
    if (string_starts_with(norm_pkg, token) || strstr(norm_pkg, token) != NULL) {
        return true;
    }
...
static void pkg_config_collect_packages(StringList *packages) {
    ...
    while (fgets(line, sizeof(line), pipe) != NULL) {
        ...
        saved = *cursor;
        *cursor = '\0';
        if (text_is_safe_pkg_name(line)) {
            string_list_push_unique(packages, line);
        }
        *cursor = saved;
    }
...
void autolib_infer(const char *root_dir, const StringList *source_paths, StringList *libs) {
    ...
    for (i = 0; i < source_paths->len; ++i) {
        ...
        if (content != NULL) {
            ...
            for (rule_index = 0; rule_index < includes.len; ++rule_index) {
                maybe_add_pkgconfig_from_include(includes.items[rule_index], libs);
            }
