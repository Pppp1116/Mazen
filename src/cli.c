...
static bool validate_standard_value(const char *value, const char *option_name, Diagnostic *diag) {
...
static bool take_option_value(int argc, char **argv, int *index, const char *arg, const char *option_name,
...
bool cli_parse(int argc, char **argv, CliOptions *options, Diagnostic *diag) {
    ...
    for (i = 1; i < argc; ++i) {
        ...
        if (strcmp(arg, "--compiler") == 0 || string_starts_with(arg, "--compiler=")) {
            const char *value = NULL;
        ...
        if (strcmp(arg, "-I") == 0 || strcmp(arg, "--include-dir") == 0 ||
            ...
            if (strcmp(arg, "-I") == 0) {
                ...
                value = argv[++i];
            } else if (!take_option_value(argc, argv, &i, arg, "--include-dir", &value, diag)) {
                return false;
            }
        ...
        if (strcmp(arg, "--src-dir") == 0 || string_starts_with(arg, "--src-dir=")) {
            const char *value = NULL;
        ...
        if (strcmp(arg, "--exclude") == 0 || string_starts_with(arg, "--exclude=")) {
            const char *value = NULL;
